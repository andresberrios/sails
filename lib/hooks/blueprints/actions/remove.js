...
module.exports = function remove(req, res) {
	...
	if (!id) {
		return res.badRequest('No id provided (primary key of the record to be removed.)');
	}
				...
				.exec({
					...
					success: function found(matchingRecord) {
						...
						if (sails.hooks.pubsub) {
							Model.publishRemove(matchingRecord[Model.primaryKey], alias, id, !sails.config.blueprints.mirror && req);
						}
